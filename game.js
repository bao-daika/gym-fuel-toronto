(function() {
    ...
    // --- LOGIC GIẢM LỰC CHO MOBILE (80%) ---
    const isMobile = /iPhone|iPad|iPod|Android/i.test(navigator.userAgent);
    const mobileForceMultiplier = isMobile ? 0.8 : 1.0; 
    ...
    window.pushWater = function(side) {
        ...
        plates.forEach(p => {
            ...
            if (dist < 230) {
                // ÁP DỤNG HỆ SỐ LỰC (0.8 CHO MOBILE / 1.0 CHO LAPTOP)
                let baseForce = (p.y > canvas.height * 0.7 ? 60 : (p.y > canvas.height * 0.35 ? 30 : 20)) * mobileForceMultiplier;
